    ...
    for u in users:
        html += f"""
            <tr>
                ...
                <td>
                    <button onclick="updatePlayer('{u['username']}')">保存</button>
                    <button onclick="updatePassword('{u['username']}')" style="margin-left:5px; background:#5a7a5a;">修改密码</button>
                </td>
            </tr>
        """
    html += """
...
<script>
    ...
    async function updatePassword(username) {
        let newPassword = prompt(`请输入用户 ${username} 的新密码：`);
