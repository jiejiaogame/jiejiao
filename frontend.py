...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; user-select: none; }
        ...
        .modal-content p, .modal-content span, .modal-content div, .modal-content td {
            color: #f0f0f0;
        }
        ...
        .city-column h3, .city-column h4 {
            color: gold;
        }
        .city-column p, .city-column span, .city-column div {
            color: #ffd966;
        }
        ...
        #ownPalaceInfo p, #ownPalaceInfo span, #ownPalaceInfo div {
            color: #ffd966;
        }
        #occupierDisplay img {
            border: 1px solid gold;
...
<div class="app" id="app" style="background-size: cover; background-position: center;">
    ...
    <div class="top-bar">
        <div class="user-info">
            ...
            <div style="display: flex; align-items: center;">
                ...
                <span id="blessingTimer" class="blessing-timer" style="display: none;"></span>
            </div>
        </div>
    ...
    <div id="authModal" class="modal" style="display: flex; z-index: 10002;"><div class="modal-content" style="width: 320px;"><h3 id="authTitle">登录</h3><div id="authForm"><input type="text" id="authUsername" placeholder="用户名" style="width:100%; margin:5px 0;"><br><input type="password" id="authPassword" placeholder="密码" style="width:100%; margin:5px 0;"><br><div id="authEmailGroup" style="display:none;"><input type="email" id="authEmail" placeholder="邮箱" style="width:100%; margin:5px 0;"></div><button id="authSubmitBtn" style="width:100%; margin:5px 0;">登录</button><button id="switchAuthBtn" style="width:100%; background: #3a5a3a;">没有账号？去注册</button></div></div></div>
        ...
        <div class="modal-content">
            ...
            <div class="pray-panel-in-modal">
                ...
                <div class="pray-buttons">
                    <div class="pray-btn" data-type="wealth">
                        <div>💰 求财</div>
                        <small style="font-size: 10px;">50%得金币</small>
                    </div>
                    <div class="pray-btn" data-type="love">
                        <div>💕 求缘</div>
                        <small style="font-size: 10px;">50%得武将</small>
                    </div>
                    <div class="pray-btn" data-type="chance">
                        <div>🍀 求机缘</div>
                        <small style="font-size: 10px;">30%得经验药水</small>
                    </div>
                </div>
                <div id="prayResult" style="margin-top: 12px; font-size: 12px; color: #ffaa66; text-align: center;"></div>
            </div>
            <button onclick="closeRecruitShop()" style="margin-top: 10px;">关闭</button>
        </div>
    </div>
...
let isFightingMusic = false;

function initMusic() {
    bgMusic = new Audio('/static/music/bg01.mp3');
    bgMusic.loop = true;
    bgMusic.volume = 0.7;
    battleMusic = new Audio('/static/music/battle.mp3');
    battleMusic.loop = true;
    battleMusic.volume = 0.7;
    winSound = new Audio('/static/music/fightwin.mp3');
    winSound.loop = false;
    winSound.volume = 0.7;
    loseSound = new Audio('/static/music/fightlose.mp3');
        ...
        volumeSlider.oninput = async (e) => {
            ...
            if (currentUser) {
                await fetch('/set_volume', {
                    ...
                    body: JSON.stringify({ username: currentUser, volume: e.target.value })
                });
            }
        };
    }
...
function playBgMusic() {
    if (currentMusic === bgMusic) return;
    stopAllMusic();
    bgMusic.currentTime = 0;
    bgMusic.play().catch(e => console.log("自动播放被阻止，等待用户交互"));
    currentMusic = bgMusic;
    isFightingMusic = false;
}
...
function playBattleMusic() {
    ...
    battleMusic.play().catch(e => console.log("自动播放被阻止"));
    currentMusic = battleMusic;
    isFightingMusic = true;
}

function playWinSound() {
    if (currentMusic === winSound) return;
    stopAllMusic();
...
function playLoseSound() {
    if (currentMusic === loseSound) return;
    stopAllMusic();
...
function addLog(msg, color = null) {
    ...
    if (color) {
        ...
        p.style.textShadow = '0 0 2px black';
    }
...
function startBlessingTimer() {
    if (blessingTimerInterval) clearInterval(blessingTimerInterval);
    blessingTimerInterval = setInterval(() => updateBlessingTimer(), 1000);
    updateBlessingTimer();
}
...
function addFriendRequestLog(fromUser, msg) {
    ...
    p.innerHTML = `${msg} 
        <button onclick="acceptFriendRequest('${fromUser}')" style="background:#5a8a5a; margin-left:10px; padding:2px 8px;">✅ 同意</button>
        <button onclick="rejectFriendRequest('${fromUser}')" style="background:#8a5a5a; margin-left:5px; padding:2px 8px;">❌ 拒绝</button>
    `;
    logDiv.appendChild(p);
    logDiv.scrollTop = logDiv.scrollHeight;
}
...
async function acceptFriendRequest(fromUser) {
    let resp = await fetch('/api/friend/accept', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: currentUser, from: fromUser })
    });
    let data = await resp.json();
    if (data.success) {
        addLog(`✅ 已添加 ${fromUser} 为好友`);
        loadFriendList();
        if (ws && ws.readyState === WebSocket.OPEN) {
            ws.send(JSON.stringify({ act: "friend_list_update", target: fromUser }));
        }
    } else {
        alert(data.msg);
    }
...
async function rejectFriendRequest(fromUser) {
    let resp = await fetch('/api/friend/reject', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: currentUser, from: fromUser })
    });
...
function connectWebSocket() {
    ...
    ws.onmessage = (e) => {
        ...
        if (data.type === "login_ok") console.log("登录成功");
            ...
            addFriendRequestLog(data.from, data.msg || `📨 ${data.from} 请求添加你为好友`);
        }
...
function sendChatMessage() {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ act: "chat", msg }));
...
function initTabs() {
    ...
    document.getElementById('formationType').onchange = async () => {
        ...
        renderFormationGrid();
        await fetch('/save_formation_type', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function loadFormation() { 
    let r = await fetch(`/get_formation?username=${currentUser}`); 
    let d = await r.json(); 
    myFormation = d.formation || []; 
    renderFormationGrid(); 
}
function renderFormationGrid() {
    ...
    let container = document.getElementById('formationGrid');
    container.innerHTML = '';
    for (let i = 0; i < 9; i++) {
        let isAvailable = availableSlots.includes(i);
        let hero = myFormation.find(f => f.position === i);
        let slotDiv = document.createElement('div');
        slotDiv.className = 'formation-slot';
        if (!isAvailable) {
            slotDiv.classList.add('disabled-slot');
            slotDiv.innerHTML = '<div class="empty-slot">🔒</div>';
        } else if (hero) {
            let heroId = heroIdMap[hero.name] || hero.name;
            slotDiv.innerHTML = `<div class="formation-hero"><img src="/static/images/heroes/${heroId}.png" onerror="this.src='/static/images/heroes/hero.png'"><div class="formation-hero-name">${hero.name}</div></div>`;
            slotDiv.onclick = () => removeFromFormation(i);
        } else {
            slotDiv.innerHTML = '<div class="empty-slot">⚡</div>';
            slotDiv.onclick = () => selectHeroForSlot(i);
        }
...
async function selectHeroForSlot(realPosition) {
    ...
    if (!activeSlots.includes(realPosition)) {
...
function clearCurrentFormation() {
    if (confirm("确定清空当前阵法下的所有站位吗？此操作不会影响其他阵法，但会清除当前已放置的武将。")) {
        myFormation = [];
        saveFormation();
        addLog("🗑️ 已清空当前阵法的所有武将");
    }
}

// ========== 宝石商店 ==========
const shopGems = [
    { type: 'strength', name: '力量宝石', icon: 'strength.png' },
    { type: 'intelligence', name: '智力宝石', icon: 'intelligence.png' },
...
function renderGemShop() {
    ...
    for (let gem of shopGems) {
        html += `
            <div class="gem-card" style="width: 150px;">
                <img src="/static/images/gems/${gem.icon}" onerror="this.src='/static/images/gems/gem.png'" style="width: 50px;">
                <div class="gem-name">${gem.name}</div>
                <div>★1星 +随机1-2</div>
                <div>价格: 1 💎/个</div>
                <div class="shop-qty-control">
                    <button class="shop-qty-down" data-type="${gem.type}">-</button>
                    <span id="shop_qty_${gem.type}" style="min-width: 30px;">1</span>
                    <button class="shop-qty-up" data-type="${gem.type}">+</button>
                </div>
                <button class="shop-buy-btn" data-type="${gem.type}" style="background: #d99e3e; width: 100%;">购买</button>
            </div>
        `;
    }
    ...
    document.querySelectorAll('.shop-qty-up').forEach(btn => {
        btn.onclick = (e) => {
            ...
            let span = document.getElementById(`shop_qty_${type}`);
            let val = parseInt(span.innerText) || 1;
            if (val < 99) span.innerText = val + 1;
        };
    });
    document.querySelectorAll('.shop-qty-down').forEach(btn => {
        btn.onclick = (e) => {
            let type = btn.dataset.type;
            let span = document.getElementById(`shop_qty_${type}`);
            let val = parseInt(span.innerText) || 1;
    ...
    document.querySelectorAll('.shop-buy-btn').forEach(btn => {
        btn.onclick = async () => {
            let type = btn.dataset.type;
            let qtySpan = document.getElementById(`shop_qty_${type}`);
            let quantity = parseInt(qtySpan.innerText) || 1;
            let resp = await fetch('/shop/buy_gem', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username: currentUser, gem_type: type, quantity: quantity })
            });
            let data = await resp.json();
            if (data.success) {
                addLog(`✅ 购买成功，获得 ${quantity} 颗${type}宝石`);
...
async function loadGems() {
    ...
    items.forEach(g => {
        ...
        div.innerHTML = `
            <div style="display:flex; flex-direction:column; align-items:center;">
                <img src="/static/images/gems/${iconFile}" onerror="this.src='/static/images/gems/gem.png'" style="width:40px; height:40px; object-fit:contain;">
                <strong style="font-size:12px;">${nameText}</strong>
                <span style="font-size:11px;">${detailText}</span>
                <span style="font-size:10px;">数量: ${g.count}</span>
                <button class="discard-gem-btn" data-id="${g.id}" style="margin-top:4px; padding:2px 8px; font-size:11px; background:#8a5a5a;">🗑️ 丢弃</button>
            </div>
        `;
        bagDiv.appendChild(div);
    });
    document.querySelectorAll('.discard-gem-btn').forEach(btn => {
        btn.onclick = async (e) => {
            e.stopPropagation();
            let gemId = btn.dataset.id;
            if (confirm("确定要丢弃一颗该宝石吗？")) {
                let resp = await fetch('/discard_gem', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: currentUser, gem_id: gemId })
                });
                let data = await resp.json();
                if (data.success) {
    ...
    function fillGemSelect(selectElement) {
        selectElement.innerHTML = '<option value="">选择宝石</option>';
        normalGems.forEach(g => {
            let attrName = g.attr === 'strength' ? '力量宝石' : (g.attr === 'intelligence' ? '智力宝石' : (g.attr === 'speed' ? '敏捷宝石' : '生命宝石'));
            selectElement.innerHTML += `<option value="${g.id}" data-star="${g.star}" data-attr="${g.attr}">${attrName} ★${g.star} +${g.value} (x${g.count})</option>`;
        });
    }
    fillGemSelect(gem1Select);
    fillGemSelect(gem2Select);
    function updateFusionHint() {
        let val1 = gem1Select.value, val2 = gem2Select.value;
        let hintDiv = document.getElementById('fusionRuleHint');
        if (!val1 || !val2) { hintDiv.innerHTML = '请选择两颗宝石'; hintDiv.style.color = '#aaa'; return; }
        if (val1 === val2) {
            let opt1 = gem1Select.options[gem1Select.selectedIndex];
            let countMatch = opt1.text.match(/x(\\d+)/);
            let count = countMatch ? parseInt(countMatch[1]) : 1;
            if (count < 2) { hintDiv.innerHTML = '❌ 该宝石数量不足2，无法融合'; hintDiv.style.color = '#ff6666'; return; }
        }
        let opt1 = gem1Select.options[gem1Select.selectedIndex];
        let opt2 = gem2Select.options[gem2Select.selectedIndex];
        let star1 = parseInt(opt1.dataset.star), star2 = parseInt(opt2.dataset.star);
        let attr1 = opt1.dataset.attr, attr2 = opt2.dataset.attr;
        ...
        if (attr1 === attr2) {
            let rates = {1: 90, 2: 70, 3: 60, 4: 40};
            let rate = rates[star1];
            hintDiv.innerHTML = `✅ 同属性融合：${star1}星+${star1}星 → ${star1+1}星（成功率${rate}%）`;
            hintDiv.style.color = '#aaffaa';
        } else {
            hintDiv.innerHTML = `⚠️ 不同属性融合：${star1}星+${star1}星 → ${Math.max(1, star1-1)}星双属性（成功率50%）`;
            hintDiv.style.color = '#ffaa66';
        }
    }
    gem1Select.onchange = updateFusionHint;
    gem2Select.onchange = updateFusionHint;
    document.getElementById('doFuseBtn').onclick = async () => {
        let gemId1 = gem1Select.value;
        let gemId2 = gem2Select.value;
        if (!gemId1 || !gemId2) { alert("请选择两颗宝石"); return; }
        if (gemId1 === gemId2) {
            let opt1 = gem1Select.options[gem1Select.selectedIndex];
            let countMatch = opt1.text.match(/x(\\d+)/);
            let count = countMatch ? parseInt(countMatch[1]) : 1;
            if (count < 2) { alert("该宝石数量不足2，无法使用同一宝石融合"); return; }
        }
        let resp = await fetch('/fuse_gems', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username: currentUser, gem_id1: gemId1, gem_id2: gemId2 })
        });
        let data = await resp.json();
        if (data.success) {
            addLog(`✨ ${data.msg}`);
    ...
    items.filter(g => g.double_attr).forEach(g => {
        decomposeSelect.innerHTML += `<option value="${g.id}">${g.name} 双属性 (x${g.count})</option>`;
    });
}
...
async function showHeroDetail(heroName) {
    ...
    for (let i = 0; i < 5; i++) {
        let gem = d.gems[i];
        let slotDiv = document.createElement('div'); slotDiv.className = 'gem-slot';
        if (gem) {
            let gemAttrName = gem.attr === 'strength' ? '⚔️力量' : (gem.attr === 'intelligence' ? '🔮智力' : (gem.attr === 'speed' ? '🏃敏捷' : '❤️生命'));
            slotDiv.innerHTML = `<div><img src="/static/images/gems/${gem.attr || 'gem'}.png" onerror="this.style.display='none'" style="width:40px;"><div style="font-size:10px;">${gemAttrName}+${gem.value}</div><button class="unequip-gem-btn" data-slot="${i}" style="font-size:10px; padding:2px 4px;">💎 卸下</button></div>`;
        } else {
            slotDiv.innerHTML = `<div><div style="width:40px; height:40px; background:#2c3e2f; border-radius:8px; margin:0 auto;"></div><div style="font-size:10px;">空</div><button class="equip-gem-btn" data-slot="${i}" style="font-size:10px; padding:2px 6px;">🔧 镶嵌</button></div>`;
        }
...
async function selectGemForSlot(heroName, slot) {
    ...
    document.querySelectorAll('#gemSelectList .gem-card').forEach(card => {
        card.onclick = async () => {
            let gemId = card.dataset.id;
            let eq = await fetch('/equip_gem', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username: currentUser, hero_name: heroName, gem_id: gemId, slot: slot })
            });
            let eqd = await eq.json();
            if (eqd.success) {
                addLog(`💎 为 ${heroName} 镶嵌了宝石`);
                modal.remove();
...
async function loadDailyTasks() {
    ...
    document.querySelectorAll('.claim-task-btn').forEach(btn => {
        btn.onclick = async () => {
            ...
            let resp = await fetch('/claim_task', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username: currentUser, task_id: taskId })
            });
            let data = await resp.json();
            if (data.success) {
                addLog(`✨ ${data.msg}`);
...
async function loadCityInfo() {
    let r = await fetch(`/user_info?username=${currentUser}`); let d = await r.json();
    if (d.success) { document.getElementById('playerLevel').innerText = d.level; document.getElementById('playerExp').innerText = d.exp; }
    let occ = await fetch(`/get_occupier?username=${currentUser}`); let occd = await occ.json();
    if (occd.occupier) {
        let occUser = await fetch(`/user_info?username=${occd.occupier}`); let occUserData = await occUser.json();
        document.getElementById('occupierName').innerHTML = `<strong>${occd.occupier}</strong> (占领者)`;
        document.getElementById('occupierAvatar').src = `/static/images/avatars/${occUserData.avatar || 'hero.png'}`;
        document.getElementById('selfReleaseBtn').style.display = 'inline-block';
    } else {
        document.getElementById('occupierName').innerHTML = '未被占领';
        document.getElementById('occupierAvatar').src = `/static/images/avatars/${d.avatar || 'hero.png'}`;
        document.getElementById('selfReleaseBtn').style.display = 'none';
    }
    loadBranchList();
    loadDailyTasks();
}
...
async function loadBranchList() {
    ...
    branchTimer = setInterval(() => { if (document.getElementById('tab-city').style.display === 'block') loadBranchList(); }, 1000);
}
function checkSelfOccupied() { loadCityInfo(); }

// ========== 访问好友碧游宫和万仙殿 ==========
async function visitFriendPalace(friendName) {
    let existingModal = document.querySelector('.modal');
    if (existingModal) existingModal.remove();
    let r = await fetch(`/friend_palace_info?username=${currentUser}&friend=${friendName}`);
    let d = await r.json();
...
async function viewFriendRecruit(friendName) {
    let info = await fetch(`/api/occupied_info?username=${currentUser}&target=${friendName}`);
    let data = await info.json();
    if (data.success && data.isOccupiedByMe && !data.canRecruit) {
        alert(`占领未满10分钟，还需等待 ${data.remainSeconds} 秒才能招募`);
        return;
    }
    let r = await fetch(`/recruit/other_shop?username=${currentUser}&target=${friendName}`);
    let d = await r.json();
    if (d.error) { alert(d.error); return; }
    let modal = document.createElement('div');
    modal.className = 'modal';
    let shopHtml = '<div class="hero-grid">';
    if (Object.keys(d.shop).length === 0) {
    ...
    if (data.isOccupiedByMe && data.canRecruit) {
        document.querySelectorAll('.hire-from-occupied').forEach(btn => {
            btn.onclick = async () => {
                ...
                let resp = await fetch('/recruit/hire_from_occupied', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ username: currentUser, target: friendName, hero_name: heroName })
                });
...
// ========== 招贤馆 ==========
async function openRecruitShop() { 
    if (!currentUser) { alert("请先登录"); return; }
    ...
    let d = await r.json(); 
    if (d.error) { alert(d.error); return; } 
    infoDiv.innerHTML = `<p>💰 金币: ${d.gold}</p><p>下次刷新: ${new Date(d.next_refresh).toLocaleTimeString()}</p>`; 
    heroesDiv.innerHTML = ''; 
    if (d.shop && Object.keys(d.shop).length > 0) {
        for (let [name, info] of Object.entries(d.shop)) { 
            let div = document.createElement('div'); 
            div.className = 'recruit-card'; 
            div.innerHTML = `<img src="/static/images/heroes/${info.hero_id}.png" onerror="this.src='/static/images/heroes/hero.png'" style="width:80px;"><br>${name}<br>★${info.star}<br>💰${info.cost}<br><button onclick="hireHero('${name}')">招募</button>`; 
            heroesDiv.appendChild(div); 
        } 
    } else {
        heroesDiv.innerHTML = '<div style="color:gold; text-align:center; padding:20px;">暂无武将，请稍后刷新</div>';
    }
    document.getElementById('recruitShopModal').style.display = 'flex'; 
    setBackgroundByTab('recruit'); 
...
function closeRecruitShop() { 
    document.getElementById('recruitShopModal').style.display = 'none'; 
    document.body.style.overflow = '';
    let mainContent = document.getElementById('mainContent');
    if (mainContent) mainContent.style.display = 'block';
    let activeTab = document.querySelector('.tab-btn.active')?.dataset.tab;
    if (activeTab) setBackgroundByTab(activeTab);
    else setBackgroundByTab('map');
}
...
// ========== 祈愿系统 ==========
async function loadPrayStatus() {
    if (!currentUser) return;
    try {
        const res = await fetch(`/pray/status?username=${currentUser}`);
        const data = await res.json();
        if (data.success) {
            const remainSpan = document.getElementById('prayRemainCount');
            const btns = document.querySelectorAll('.pray-btn');
            if (remainSpan) {
...
async function pray(prayType) {
    ...
    try {
        const res = await fetch('/pray/burn_incense', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username: currentUser, pray_type: prayType })
        });
        const data = await res.json();
        if (data.success) {
            resultDiv.innerHTML = `✅ ${data.msg}`;
            await loadUserResources();
            if (data.reward && data.reward.type === 'hero') {
                const recruitRes = await fetch(`/recruit/shop?username=${currentUser}`);
                const shopData = await recruitRes.json();
                if (!shopData.error) {
                    const heroesDiv = document.getElementById('recruitHeroes');
                    if (heroesDiv && shopData.shop) {
                        heroesDiv.innerHTML = '';
                        for (let [name, info] of Object.entries(shopData.shop)) {
                            let div = document.createElement('div');
                            div.className = 'recruit-card';
                            div.innerHTML = `<img src="/static/images/heroes/${info.hero_id}.png" onerror="this.src='/static/images/heroes/hero.png'" style="width:80px;"><br>${name}<br>★${info.star}<br>💰${info.cost}<br><button onclick="hireHero('${name}')">招募</button>`;
                            heroesDiv.appendChild(div);
...
function renderFriendList() { 
    ...
    let noMsg = document.getElementById('noFriendsMsg'); 
    if (!container) return; 
    if (friendsList.length === 0) { container.innerHTML = ''; noMsg.style.display = 'block'; return; } 
    noMsg.style.display = 'none'; 
    container.innerHTML = ''; 
    let displayFriends = friendsList.slice(0, 50); 
    for (let f of displayFriends) { 
        let div = document.createElement('div'); 
        div.className = 'friend-item'; 
        div.innerHTML = `<div style="display:flex; align-items:center; gap:8px; flex:1; cursor:pointer;" onclick="showFriendActions('${f.name}')"><img src="/static/images/avatars/${f.avatar || 'hero.png'}" class="friend-avatar" onerror="this.src='/static/images/avatars/hero.png'"><div><div class="friend-name">${f.name}</div><div class="friend-status">${f.online ? '🟢 在线' : '⚫ 离线'}</div></div></div><div class="friend-actions"><button onclick="showFriendActions('${f.name}')">📋</button></div>`; 
        container.appendChild(div); 
    } 
}
...
function privateChat(friendName) {
    ...
    if (chatInput) { chatInput.value = `@${friendName} `; chatInput.focus(); }
    let modal = document.querySelector('.modal');
    if (modal) modal.remove();
    addLog(`💬 开始与 ${friendName} 私聊，输入消息后发送（会自动提醒对方）`);
}
...
async function robOccupied(friendName) {
    let m = document.querySelector('.modal');
    if (m) m.remove();
    let r = await fetch('/rob_occupied', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: currentUser, target: friendName })
    });
    let d = await r.json();
    if (d.success) {
        addLog(`⚔️ ${d.msg}`);
...
async function showAvatarModal() { 
    ...
    for (let av of d.avatars) { 
        let div = document.createElement('div'); 
        div.className = 'recruit-card'; 
        div.innerHTML = `<img src="/static/images/avatars/${av}" style="width:60px;" onerror="this.parentElement.remove()"><div>${av}</div>`; 
...
async function loadPendingRequests() {
    ...
    if (data.success && data.requests) {
        for (let req of data.requests) {
            addFriendRequestLog(req.from, `📨 ${req.from} 请求添加你为好友（离线期间）`);
        }
    }
}
